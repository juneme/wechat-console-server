...
适用于已安装 Docker Compose 的 Linux 服务器。容器默认只绑定 `127.0.0.1:8791`，可在 `.env` 中通过 `YUNOE_HOST_PORT` 覆盖宿主机端口，再由反向代理对外提供服务。
...
静态图片、草稿和临时图片 Key 已全部移除。每次配对签发独立令牌，每个用户保留最近使用的 16 枚令牌；修改密码会撤销该用户的全部客户端令牌。
